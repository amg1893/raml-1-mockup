...
module.exports = function(params, callback) {
  ...
  function processMultipleExamples(examplesSchema) {
    return examplesSchema.map(function(exampleSchema) {
      return exampleSchema.structuredValue;
    });
  }
  ...
  raml_parser.loadApi(params.raml).then(function(data) {
    ...
    try {
      ...
      _.each(data.types, function(obj) {
        _.each(obj, function(schema, ref) {
          api.definitions[ref] = JSON.parse(schema.type);
        });
      });
        ...
        _.each(api.resources, function(resource, path) {
          ...
          _.each(resource, function(responses, method) {
            ...
            app.route(route)[method](function(req, res) {
              ...
              var reqForceExample = req.query._forceExample === 'true' || false;
              ...
              try {
                ...
                var responseExample = responses[reqStatus].example || null;

                var typeExample = null;
                var type = responses[reqStatus].type || undefined;
                var is_list_type = responses[reqStatus].is_list_type;

                if(typeof type !== 'undefined') {
                  if(!is_list_type) {
                    var potentialSingleExample = type.examples ? pickOne(processMultipleExamples(type.examples)) : null;
                    typeExample = type ? potentialSingleExample || type.example : null;
                  } else {
                    var potentialExamples = type.examples ? processMultipleExamples(type.examples) : null;
                    typeExample = type ? potentialExamples || [type.example] : null;
                  }
                }

                if (reqForceExample && !(responseExample || typeExample)) {
                  throw new Error('missing example for ' + req.url + ' (' + reqStatus + ')');
                }

                if (!reqForceExample) {
                  var schema = api.definitions[responses[reqStatus]._ref] || null;
                  typeExample = schema ? jsonfaker(schema, refs) : '{}';
                }

                if(typeof responseExample !== 'undefined' && typeof responseExample === 'string') {
                  responseExample = JSON.parse(responseExample);
                }
                res.statusCode = reqStatus;
                res.json(responseExample || typeExample);
              } catch (e) {
                res.statusCode = 500;
